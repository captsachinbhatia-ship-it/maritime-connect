...
import { Loader2, CalendarIcon, Search, FileText } from 'lucide-react';
...
export interface LogInteractionModalProps {
  ...
  onClose: () => void;
  onSuccess: () => void;
  /** If provided, skip the contact selector (contact page mode) */
  contactId?: string;
  contactName?: string;
...
export function LogInteractionModal({
  ...
  const needsContactSelector = !fixedContactId;

  // Contact selector state (dashboard mode)
  const { contacts: assignedContacts, loading: assignedLoading } = useAssignedContacts(
    crmUserId,
    isOpen && needsContactSelector && !isAdmin && !crmLoading,
  );
  ...
  const [enquiryPopoverOpen, setEnquiryPopoverOpen] = useState(false);

  const effectiveContactId = fixedContactId || selectedContact?.id || null;
  const effectiveContactName = fixedContactId ? (fixedContactName || 'Contact') : (selectedContact?.full_name || '');
  ...
  useEffect(() => {
    if (isOpen && needsContactSelector && isAdmin) fetchAdminContacts();
  }, [isOpen, needsContactSelector, isAdmin, fetchAdminContacts]);

  // Fetch enquiries linked to selected contact
  useEffect(() => {
    ...
    const fetchEnquiries = async () => {
      try {
        const { data } = await supabase
          .from('enquiries')
          .select('id, enquiry_number, subject, cargo_type, loading_port, discharge_port')
          .or(`contact_id.eq.${effectiveContactId},source_contact_id.eq.${effectiveContactId}`)
          .eq('is_draft', false)
          .is('deleted_at', null)
  ...
  // Pre-select enquiry
  useEffect(() => {
    if (preselectedEnquiryId && isOpen) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Governance: non-admin must have active assignment (dashboard mode)
    if (needsContactSelector && !isAdmin) {
      ...
      if (!assignment || assignment.length === 0) {
        toast({ title: 'Not Allowed', description: 'You can only log interactions for contacts assigned to you.', variant: 'destructive' });
        return;
      }
    }
    ...
    <Dialog open={isOpen} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Contact Selector (dashboard mode only) */}
          {needsContactSelector && (
                    ...
                    {selectedContact ? (
                      <span className="truncate">
                        ...
                        {selectedContact.company_name && (
                          <span className="text-muted-foreground ml-1">({selectedContact.company_name})</span>
                        )}
                      </span>
                    ) : (
                      ...
                      <CommandGroup>
                        {contacts.map((c) => (
                          <CommandItem
                            ...
                            onSelect={() => { setSelectedContact(c); setContactPopoverOpen(false); }}
                          >
          ...
          <div className="space-y-2">
            ...
            <Input
              value={subject}
              onChange={(e) => { setSubject(e.target.value); setValidationMessage(''); }}
          ...
          <div className="space-y-2">
            ...
            <Textarea
              value={notes}
              onChange={(e) => { setNotes(e.target.value); setValidationMessage(''); }}
          ...
          {effectiveContactId && (
            <div className="space-y-2">
              <Label>Linked Enquiry</Label>
              <Popover open={enquiryPopoverOpen} onOpenChange={setEnquiryPopoverOpen}>
                <PopoverTrigger asChild>
                  <Button variant="outline" role="combobox" className="w-full justify-between font-normal">
                    {selectedEnquiry ? (
                      <span className="truncate">{selectedEnquiry.enquiry_number} — {selectedEnquiry.subject || selectedEnquiry.cargo_type || 'Enquiry'}</span>
                    ) : (
                      <span className="text-muted-foreground">Select enquiry (optional)...</span>
                    )}
                    ...
                    <CommandList className="max-h-[250px] overflow-y-auto pointer-events-auto">
                      ...
                      <CommandGroup>
                        {enquiries.map((enq) => (
                          <CommandItem
                            key={enq.id}
                            value={`${enq.enquiry_number} ${enq.subject || ''} ${enq.cargo_type || ''} ${enq.loading_port || ''}`}
                            onSelect={() => { setSelectedEnquiryId(enq.id); setEnquiryPopoverOpen(false); }}
                          >
                            <span className="font-medium">{enq.enquiry_number}</span>
                            <span className="ml-2 text-xs text-muted-foreground truncate">
                              {[enq.cargo_type, enq.loading_port, enq.discharge_port].filter(Boolean).join(' · ') || enq.subject || ''}
                            </span>
                          </CommandItem>
                        ))}
                      </CommandGroup>
              ...
              {/* Open Enquiry button — directly below the selector */}
              {selectedEnquiry && (
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-sm text-muted-foreground truncate">
                    {selectedEnquiry.enquiry_number} — {selectedEnquiry.subject || selectedEnquiry.cargo_type || ''}
                  </span>
                  <Button
                    type="button"
                    variant="link"
                    ...
                    onClick={() => {
                      handleClose();
                      navigate(`/enquiries/${selectedEnquiryId}`);
                    }}
                  >
                    Open Enquiry →
                  </Button>
          ...
          <div className="rounded-lg border p-4 space-y-3">
            <div className="flex items-center space-x-2">
              ...
              />
              <label htmlFor="log-followup" className="text-sm font-medium leading-none cursor-pointer">Set Follow-up Reminder</label>
            </div>
            {!effectiveContactId && (
